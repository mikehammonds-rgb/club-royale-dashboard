...
> **Retired historical workflow.** This document describes the former standalone Claude artifact implementation. Do not use it to update, publish, or deploy the current dashboard. The canonical source is `mikehammonds-rgb/club-royale-dashboard` on GitHub, the authoritative procedure is the repository-root `WORKFLOW.md`, and production is the existing ChatGPT Site recorded in `AI_STATE.md`. The material below is retained only as research and decision history.

## The two former artifact pages (historical)

Both were Claude-hosted artifacts. They are retired and are not the current production dashboard.
...
**Do not update or republish either artifact.** Make all current changes in the canonical GitHub repository and follow the root `WORKFLOW.md`. Publishing, when separately requested, targets the existing ChatGPT Site rather than these artifact URLs.
...
**Former artifact process:** the retired Finder was regenerated by editing the `add(...)` lines and
`OFFERS`/`BENEFITS` dicts in `build_data.py`, then re-running and re-merging it. Do not use this process for the current dashboard.
...
6. Historically, this updated the artifact dashboard and Finder. For the current product, stop using this sequence and follow the root `WORKFLOW.md` instead.
...
4. Historically, the result was appended to the artifact `searchHistory`. For the current product, follow the repository-root workflow and do not republish the artifacts.
...
## Historical artifact verification method
